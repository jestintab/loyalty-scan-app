...
void main() {
  group('HttpApiClient', () {
    test('login posts the identifier and password, and returns the session',
        () async {
      ...
      final session =
          await client.login(identifier: 'sam@shop.qa', password: 'pw');
    ...
    test('an authenticated call sends the bearer token from the reader',
        () async {
      final sent = <http.Request>[];
      final client = HttpApiClient(
        baseUrl: 'https://api.test',
    ...
    test('a card id with a slash in it cannot escape its path segment',
        () async {
      final sent = <http.Request>[];
      final client = HttpApiClient(
        baseUrl: 'https://api.test',
    ...
    test('a socket failure becomes a network ApiException, not a raw error',
        () async {
      final client = HttpApiClient(
        baseUrl: 'https://api.test',
